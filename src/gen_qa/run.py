...
llm_client = OpenAI(
    api_key=os.environ["DOUBAO_API_KEY"], base_url=os.environ["DOUBAO_BASE_URL"]
)
...
def chat(prompt, max_retry=3, debug=False, temperature=0.85, top_p=0.95):
    ...
    while max_retry > 0:
        try:
            ...
            if debug:
                print(
                    f"{str(e)}, remain retry: {max_retry}, sleeping {sleep_seconds}s {prompt}"
                )
...
def gen_qa(splitted_docs, prompt_tmpl, qa_ckpt_filename, expand=False):
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=MAX_WORKERS) as executor:
        if expand:
            futures = {
                doc.page_content: executor.submit(
                    chat, build_qa_prompt(prompt_tmpl, doc.page_content), 3, True
                )
                for doc in splitted_docs
                if doc.metadata["unique_id"] not in qa_ckpt
            }
        else:
            futures = {
                ...
                and doc.metadata["unique_id"] not in qa_ckpt
            }
...
if __name__ == "__main__":
    ...
    for unique_id, info in expand_qa_dict.items():
        ...
        expand_questions = [
            re.sub(r"^\d[.. ]", "", item).strip() for item in expand_questions
        ]
    ...
    for idx, answer in enumerate(unique_test_answers):
        test_answer_docs.append(
            Document(page_content=answer, metadata={"unique_id": str(idx)})
        )

    print("待抽取关键词docs数：", len(test_answer_docs))
    keywords_dict = gen_qa(
        test_answer_docs, KEYWORDS_PROMPT_TPL, TEST_KEYWORDS_PATH, expand=True
    )
